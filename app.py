...
# Initialize active memory for tables so AI can modify them
...
def parse_config_updates(user_text, key):
    ...
    For add_shift include: "Task ID" (generate one), "Shift Name", "Start Time" (HH:MM string), "End Time" (HH:MM string), "Req Headcount" (integer).
    For add_physician include: "Provider ID" (generate one), "Name", "Max Total" (integer), "Max Nights" (integer), "Max Weekends" (integer).
    Text: "{user_text}"
    """
    return json.loads(model.generate_content(prompt).text)
...
def physician_view():
    with st.sidebar:
        ...
        # Added API key for Physician AI tools
    ...
    with tab2:
        ...
        # --- NEW: PHYSICIAN AI INPUT ---
        with st.container(border=True):
            st.markdown("🤖 **AI Assistant: Submit via text**")
            phys_req = st.text_input("Example: 'I need Day 2 off from 8am to 5pm' or 'I can't work Day 5 from 14:00 to 22:00'")
                ...
                if not api_key: 
                    ...
                    with st.spinner("Parsing request..."):
                        try:
                            ...
                            st.error(f"AI failed to understand. Try formatting differently. Error: {e}")
...
def admin_view():
    ...
    with tab_config:
        st.markdown("Configure your hospital's capacity and shift requirements.")
        ...
        with st.container(border=True):
            ...
            with col_txt:
                ai_config_cmd = st.text_input("Instruction:", placeholder="E.g., 'Add a Swing Shift from 3pm to 11pm needing 2 headcount' or 'Add Dr. Miller with ID DOC-04 max 20 shifts and 2 nights'")
            with col_btn:
                st.write("") 
                            ...
                            try:
                                ...
                                for item in parsed.get("updates", []):
                                    if item.get("type") == "add_shift":
                                        new_shifts.append({
                                            "Task ID": item.get("Task ID", "TSK-AI"),
                                            "Shift Name": item.get("Shift Name", "New Shift"),
                                            "Start Time": datetime.datetime.strptime(item.get("Start Time", "00:00"), "%H:%M").time(),
                                            "End Time": datetime.datetime.strptime(item.get("End Time", "00:00"), "%H:%M").time(),
                                            "Req Headcount": item.get("Req Headcount", 1)
                                        })
                                    elif item.get("type") == "add_physician":
                                        new_docs.append({
                                            "Provider ID": item.get("Provider ID", "DOC-AI"),
                                            "Name": item.get("Name", "New Doc"),
                                            "Max Total": item.get("Max Total", 0),
                                            "Max Nights": item.get("Max Nights", 0),
                                            "Max Weekends": item.get("Max Weekends", 0)
                                        })
                                if new_shifts:
                                    st.session_state.shifts_df = pd.concat([st.session_state.shifts_df, pd.DataFrame(new_shifts)], ignore_index=True)
                                if new_docs:
                                    st.session_state.physicians_df = pd.concat([st.session_state.physicians_df, pd.DataFrame(new_docs)], ignore_index=True)
                                st.toast("Database tables updated!", icon="✅")
                                st.rerun()
                            except Exception as e:
                                st.error(f"Failed to process command. Error: {e}")
        ...
        carryover_docs = st.multiselect("Boundary Management (Quarter Carryover)", physicians_list, help="Select providers who worked the final overnight shift of the previous quarter.")

    # Process Data (Crash-Proofed for empty Streamlit cells)
    shift_reqs = {r["Shift Name"]: int(r["Req Headcount"]) if pd.notna(r.get("Req Headcount")) else 0 for _, r in edited_shifts.iterrows() if pd.notna(r.get("Shift Name")) and str(r.get("Shift Name")).strip()}
    shift_times = {r["Shift Name"]: {"start": r["Start Time"], "end": r["End Time"]} for _, r in edited_shifts.iterrows() if pd.notna(r.get("Shift Name")) and str(r.get("Shift Name")).strip()}
    shift_ids = {r["Shift Name"]: r["Task ID"] for _, r in edited_shifts.iterrows() if pd.notna(r.get("Shift Name")) and str(r.get("Shift Name")).strip()}
    shifts_list = list(shift_reqs.keys())

    p_limits = {r["Name"]: int(r["Max Total"]) if pd.notna(r.get("Max Total")) else 0 for _, r in edited_physicians.iterrows() if pd.notna(r.get("Name")) and str(r.get("Name")).strip()}
    n_limits = {r["Name"]: int(r["Max Nights"]) if pd.notna(r.get("Max Nights")) else 0 for _, r in edited_physicians.iterrows() if pd.notna(r.get("Name")) and str(r.get("Name")).strip()}
    w_limits = {r["Name"]: int(r["Max Weekends"]) if pd.notna(r.get("Max Weekends")) else 0 for _, r in edited_physicians.iterrows() if pd.notna(r.get("Name")) and str(r.get("Name")).strip()}
    p_ids = {r["Name"]: r["Provider ID"] for _, r in edited_physicians.iterrows() if pd.notna(r.get("Name")) and str(r.get("Name")).strip()}
